...
to setup
  ...
  ;; signals/demands
  set broodingSignal random 500
  set defenseSignal random 500
  set foragingSignal random 500
...
to do-task  ;; turtle procedure
  ...
  ;; Update signal with these values of xij
...
to brooding-task  ;; turtle procedure
  ...
  set broodingThresh max list 0 broodingThresh
...
to foraging-task
  ...
  ;;update thresholds
  set foragingThresh foragingThresh - learning
  set foragingThresh max list 0 foragingThresh
...
to defense-task
  ...
  set defenseThresh max list 0 defenseThresh
;  fd 1
end
...
to update-signal  ;; turtle procedure
  ...
  ask turtles[
    if color = violet [
      set broodCount broodCount + 1
      set broodxsum broodxsum + broodFraction
    ]
    if color = green [
      set forageCount forageCount + 1
      set foragexsum foragexsum + forageFraction
    ]
    if color = yellow [
      set defenseCount defenseCount + 1
      set defensexsum defensexsum + defenseFraction
    ]
    set antCount antCount + 1
  ]
  if broodCount > 0 [
    set broodingSignal (broodingSignal + .5 - (2 / antCount) * broodxsum )
  ]
  if forageCount > 0 [
    set foragingSignal (foragingSignal + .5 - (2 / antCount) * foragexsum )
  ]
  if defenseCount > 0 [
    set defenseSignal (defenseSignal + .5 - (2 / antCount) * defensexsum )
  ]
end


;to brooding-task  ;; brooding
;  ifelse nest?
...
;    [ set pcolor scale-color green 0 0.1 5 ] ]
;end
