...
;  ]
;  [
;  set broodingProbability (broodingSignal ^ 2)* (1 / max list 1 (distancexy 0 0) ^ 2 )/ (broodingSignal ^ 2 + broodingThresh ^ 2 + (distancexy 0 0) ^ 2)
;  set foragingProbability (foragingSignal ^ 2)* (1 / max list 1 (distancexy (-0.6 * max-pxcor) (-0.6 * max-pycor)) ^ 2 )/ (foragingSignal ^ 2 + foragingThresh ^ 2 + (distancexy (-0.6 * max-pxcor) (-0.6 * max-pycor)) ^ 2)
;  set defenseProbability (defenseSignal ^ 2)* (1 / max list 1 (distancexy (-0.8 * max-pxcor) (0.8 * max-pycor)) ^ 2)/ (defenseSignal ^ 2 + defenseThresh ^ 2 + (distancexy (-0.8 * max-pxcor) (0.8 * max-pycor)) ^ 2)
;;  ]
...
;  run (chosen-task)
    ...
    ifelse random-float 1 > broodingProbability
[brooding-task]
[
  ifelse random-float 1 > foragingProbability
  [foraging-task]
  [
    if random-float 1 > defenseProbability
    [defense-task]
  ]
]
;
  ...
  ; task fractions for task signal - Xij
...
; this can be used to set up different patches for each task
...
;; TO-DO: have them return to the nest
...
;    set antCount forageCount + defenseCount + broodCount

      set antCount antCount + 1
    ]
  ]
  set broodingSignal (broodingSignal + random-float 1 - (3 / antCount) * broodxsum )
  set broodingSignal max list .1 broodingSignal

  set foragingSignal (foragingSignal + random-float 1 - (3 / antCount) * foragexsum )
  set foragingSignal max list .1 foragingSignal

  set defenseSignal (defenseSignal + random-float 1 - (3 / antCount) * defensexsum )
